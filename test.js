...
var DysponentSurowcowy = {
	// USTAWIENIA DOMYSLNE
	resourcesFillTo: [15000, 20000, 15000],		// wypełniaj do tej wartości
	resourcesSafeguard: [7000, 10000, 7000],	// zabezpieczenie w surowcach, wypelniane priorytetowo
...
(async function (TribalWars) {
    ...
    const Debugger = {
		...
		verifyTransports: function (transports) {
			Debugger.logLine('verifyTransports()');
			Debugger.logLine(transports.length);
			for (var i=0; i<transports.length; i++) {
				Debugger.logLine( '' + i + '	(' + transports[i].resources[0] + ',' + transports[i].resources[1] + ',' + transports[i].resources[2] + ')' + ' ' + transports[i].traders + ' origin: ' + transports[i].origin.ID + ' destination: ' + transports[i].destination.ID );
				if(transports[i].resources[0] < 0) { throw 'aaa' }
				if(transports[i].resources[1] < 0) { throw 'bbb' }
				if(transports[i].resources[2] < 0) { throw 'ccc' }
				if((transports[i].resources[0]+transports[i].resources[1]+transports[i].resources[2])/1000 != transports[i].traders) { 
					console.log(transports);
					throw 'ddd (' + transports[i].resources[0] + ',' + transports[i].resources[1] + ',' + transports[i].resources[2] + ')' + ' ' + transports[i].traders + ' origin: ' + transports[i].origin.ID + ' destination: ' + transports[i].destination.ID;
				}
			}
		}
	};
    ...
    const Script = {
		...
		transportsHandler: {
			...
			addTransport: function (_transports, _resources, _origin, _destination) {
				...
				let transport = _transports[l-1];

				if ((transport.resources[0]+transport.resources[1]+transport.resources[2])/1000 != transport.traders) {
					throw 'traders != res/1000 : (' + transport.resources[0] + ',' + transport.resources[1] + ',' + transport.resources[2] + ')' + ' ' + transport.traders;
				}

				Debugger.logLine('NEW TRANSPORT: (' + transport.resources[0] + ',' + transport.resources[1] + ',' + transport.resources[2] + ')' + ' traders:' + transport.traders);
			...
			reduceTransport: function (reduction, transport) {
				Debugger.logLine('reduceTransport()');
				console.log(transport);
				Debugger.verifyTransports(Script.transportsHandler.generalTransports);
				Debugger.logLine('origin: ' + transport.origin.ID + ' destination: ' + transport.destination.ID);

				if (reduction[0]<0 || reduction[1]<0 || reduction[2]<0) {
					throw '' + reduction[0] + ' ' + reduction[1] + ' ' + reduction[2];
				}

				Debugger.logLine('resources reduction :' + reduction[0] + ' ' + reduction[1] + ' ' + reduction[2]);
				Debugger.logLine('traders reduction: ' + ((reduction[0] + reduction[1] + reduction[2]) / 1000));
				Debugger.logLine('traders: ' + transport.traders);
				Debugger.logLine('resources: ' + transport.resources[0] + ',' + transport.resources[1] + ',' + transport.resources[2]);

				Script.villagesHandler.updateVillages(transport, -1);
				Debugger.verifyTransports(Script.transportsHandler.generalTransports);
				Debugger.logLine('a');
				transport.traders = transport.traders - ((reduction[0] + reduction[1] + reduction[2]) / 1000);
				for (var i = 0; i<Script.transportsHandler.generalTransports.length; i++) {
					let transports = Script.transportsHandler.generalTransports;
					Debugger.logLine( '' + i + '	(' + transports[i].resources[0] + ',' + transports[i].resources[1] + ',' + transports[i].resources[2] + ')' + ' ' + transports[i].traders + ' origin: ' + transports[i].origin.ID + ' destination: ' + transports[i].destination.ID );
				}
				Debugger.logLine('b');
				for (var k=0; k<3; k++) {
					transport.resources[k] = transport.resources[k] - reduction[k];
				}
				Debugger.logLine('c');
				for (var i = 0; i<Script.transportsHandler.generalTransports.length; i++) {
					let transports = Script.transportsHandler.generalTransports;
				...
				Debugger.verifyTransports(Script.transportsHandler.generalTransports);
				Debugger.logLine('end of reduceTransport()');
			},
			...
			optimization: {
				...
				relayThroughBrokers () {
					...
					for (var i=0; i<transports.length; i++) {
						...
						if ((transports[i].resources[0]+transports[i].resources[1]+transports[i].resources[2])/1000 != transports[i].traders) {
							console.log(transports);
							throw 'A: traders != res/1000 : (' + transports[i].resources[0] + ',' + transports[i].resources[1] + ',' + transports[i].resources[2] + ')' + ' ' + transports[i].traders;
						}
						...
						for (var j=0; j<potBrokers.length; j++) {
							...
							Debugger.logLine('' + res[0]+' '+res[1]+' '+res[2]);
							Debugger.verifyTransports(transports);
							...
							if ((transports[i].resources[0]+transports[i].resources[1]+transports[i].resources[2])/1000 != transports[i].traders) {
								console.log(transports);
								throw 'B: traders != res/1000 : (' + transports[i].resources[0] + ',' + transports[i].resources[1] + ',' + transports[i].resources[2] + ')' + ' ' + transports[i].traders;
							}
							...
							if ((transports[l-1].resources[0]+transports[l-1].resources[1]+transports[l-1].resources[2])/1000 != transports[l-1].traders) {
								console.log(transports);
								throw 'C: traders != res/1000 : (' + transports[l-1].resources[0] + ',' + transports[l-1].resources[1] + ',' + transports[l-1].resources[2] + ')' + ' ' + transports[l-1].traders;
							}
							...
							if ((transports[l-1].resources[0]+transports[l-1].resources[1]+transports[l-1].resources[2])/1000 != transports[l-1].traders) {
								console.log(transports);
								throw 'D: traders != res/1000 : (' + transports[l-1].resources[0] + ',' + transports[l-1].resources[1] + ',' + transports[l-1].resources[2] + ')' + ' ' + transports[l-1].traders;
							}
